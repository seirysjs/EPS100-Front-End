<template>
  <div class="container">
    ...
    <div class="block has-background-light p-5">
      <div class="field is-grouped is-grouped-left">
        ...
        <div v-if="form_part != 0" class="control">
          <button class="button is-light" @click="form_part -= 1">
            Grįžti
          </button>
        </div>
        <div v-if="form_part != 3" class="control">
          <button
            ...
            @click="
              () => {
                if (product_class_id != 0 && form.worker_id != 0)
                  form_part += 1;
    ...
    <div v-if="form_part == 0">
      ...
      <div class="control">
        <label class="label" for="product_class_id"
          >Pasirinkite bloko markę:</label
        >
    ...
    <div v-if="form_part == 1">
      ...
      <div class="control">
        <div class="field is-grouped">
          <div class="control">
            <div>
              <label class="label" for="indexSelectedBlocks"
                >Blokų sąrašas</label
              >
              <select
              ...
              >
                <option
                  v-for="(block, index) in product_classes_map[product_class_id]
                    .blocks"
                  v-bind:key="index"
                  :value="index"
                >
          ...
          <div class="control" style="margin: auto">
            ...
            <button
              ...
              @click="transferSelectedBlockToGroup()"
            >
              🡒
            </button>
            &nbsp;
            ...
            <button
              class="button is-light"
              @click="transferSelectedBlockFromGroup()"
            >
              🡐
            </button>
            &nbsp;
          ...
          <div class="control">
            <label class="label" for="indexSelectedBlocksGroup"
              >Pasirinkti blokai</label
            >
            <div>
              <select
    ...
    <div v-if="form_part == 2">
      ...
      <table class="table">
        ...
        <tbody>
          <tr
            v-for="(cut, index) in product_classes_map[product_class_id].cuts"
            v-bind:key="index"
          >
            ...
            <td>
              <div class="select">
                ...
                >
                  ...
                  <option
                    v-for="(blueprint, index) in product_classes_map[
                      product_class_id
                    ].blueprints"
                    v-bind:key="index"
              ...
              <div class="field is-grouped is-grouped-right">
                <div class="control" style="margin-top: 10px">
                  ...
                  ><span class="tag is-light">{{
                    product_classes_map[product_class_id].volume
                  }}</span
                  >&nbsp;<b>(m3)</b>
                </div>
                <div class="control">
  ...
  methods: {
        ...
        fetchBlueprints().then((res) => {
          ...
          fetchProductClasses().then((res) => {
            ...
            this.product_classes.forEach((productClass) => {
              ...
              this.product_classes_map[productClass.product_class_id].blocks =
                this.blocks.filter(
                  (block) =>
                    block.product_class_id == productClass.product_class_id
                );
              this.product_classes_map[
                productClass.product_class_id
              ].blueprints = this.blueprints.filter(
                (blueprint) =>
                  blueprint.product_class_id == productClass.product_class_id
              );
            });
          });
        });
    ...
    addCut() {
      this.product_classes_map[this.product_class_id].cuts.push({
        product_class:
          this.product_classes_map[this.product_class_id].productClass.name,
        blueprint_id: 0,
        blueprint: {},
        quantity: 0,
        quantityM3: 0,
    ...
    removeCut(index) {
      if (this.product_classes_map[this.product_class_id].cuts.length > 1)
        this.product_classes_map[this.product_class_id].cuts.splice(index, 1);
    },
    ...
    getTotalVolume() {
      ...
      for (
        ...
        productRowIndex <
        this.product_classes_map[this.product_class_id].cuts.length;
        productRowIndex++
      ) {
        const productRow =
          this.product_classes_map[this.product_class_id].cuts[productRowIndex];
        volume += productRow.quantityM3;
      }
      this.product_classes_map[this.product_class_id].volume =
        Math.round(volume * 1000) / 1000;
    },

    transferSelectedBlockToGroup() {
      if (
        !this.product_classes_map[this.product_class_id].blocks[
          this.indexSelectedBlocks
        ]
      )
        return;
      this.product_classes_map[this.product_class_id].blocksSelected.push(
        this.product_classes_map[this.product_class_id].blocks[
          this.indexSelectedBlocks
        ]
      );
      this.product_classes_map[this.product_class_id].blocks.splice(
        this.indexSelectedBlocks,
        1
      );
    },
    ...
    transferSelectedBlockFromGroup() {
      if (
        !this.product_classes_map[this.product_class_id].blocksSelected[
          this.indexSelectedBlocksGroup
        ]
      )
        return;
      this.product_classes_map[this.product_class_id].blocks.push(
        this.product_classes_map[this.product_class_id].blocksSelected[
          this.indexSelectedBlocksGroup
        ]
      );
      this.product_classes_map[this.product_class_id].blocksSelected.splice(
        this.indexSelectedBlocksGroup,
        1
      );
    },
    ...
    onChangeBlueprint(index) {
      ...
      this.product_classes_map[this.product_class_id].cuts[index].blueprint =
        this.blueprints.find(
          (blueprint) => blueprint.blueprint_id == cut.blueprint_id
        );

      if (
        this.product_classes_map[this.product_class_id].cuts[index]
          .blueprint_id != 0
      )
        fetchBlueprint(cut.blueprint_id).then((res) => {
          ...
          this.product_classes_map[this.product_class_id].cuts[
            index
          ].quantityM3 =
            Math.round(
              this.product_classes_map[this.product_class_id].cuts[index]
      ...
      if (
        ...
        this.product_classes_map[this.product_class_id].cuts[index].quantityM3 =
          Math.round(
            this.product_classes_map[this.product_class_id].cuts[index]
              .quantity *
              this.product_classes_map[this.product_class_id].cuts[index].step *
              1000
          ) / 1000;
        this.getTotalVolume();
      }
    ...
    onChangeQuantity(index) {
      ...
      if (
        ...
        fetchBlueprint(cut.blueprint_id).then((res) => {
          ...
          this.product_classes_map[this.product_class_id].cuts[
            index
          ].quantityM3 = Math.round(m3 * cut.quantity * 1000) / 1000;
          this.getTotalVolume();
        });
    ...
    postForm() {
      ...
      this.form.blocks =
        this.product_classes_map[this.form.product_class_id].blocksSelected;
      this.form.cuts =
        this.product_classes_map[this.form.product_class_id].cuts;
