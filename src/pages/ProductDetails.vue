    ...
    >
      <tab name="Užsakymai">
        ...
        <data-table
          :rowsPerPage="10"
          :list="orderItems"
          @dataSlice="(data) => (this['data' + 'Order'] = data)"
        >
          <template #head>
            <tr>
              <th>Užsakymo Nr.</th>
          ...
          <template #body>
            <tr v-for="order in dataOrder" v-bind:key="order.order_id">
              ...
              <td>
                <span class="tag is-light">x{{ order.count }}</span>
              </td>
              <td>
                <div class="field is-grouped">
                  <div class="control">
                    <div class="tag">
                      {{ order.blueprint.product_class.name }}
                    </div>
                    <br />
                    <div class="tag">
                      {{ order.blueprint.product_size.x_mm }}
                    </div>
                    x
                    <div class="tag">
                      {{ order.blueprint.product_size.y_mm }}
                    </div>
                    x
                    <div class="tag">
                      {{ order.blueprint.product_size.z_mm }}
                    </div>
                  </div>
                </div>
              </td>
              <td>
                <span class="tag is-light is-danger">{{
                  countInvWarehouseItemVolume(order)
                }}</span>
              </td>
...
import {
  ...
  fetchOrderItemsWithFulfillsByBlueprint,
  fetchOrder,
} from "@/lib/api";
import {
