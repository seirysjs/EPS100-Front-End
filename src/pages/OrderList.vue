          ...
          <tr v-for="order in data" v-bind:key="order.order_id">
            ...
            <td>
              <span
                class="tag is-light"
                :class="{
                  'is-danger': order.status == 'void',
                  'is-warning': order.status == 'wip',
                  'is-success': order.status == 'done',
                }"
                >{{ ltOrderStatus(order.status) }}</span
              >
            </td>
            <td>
              <div class="tag">{{ mapOrderItemsByBlueprints(order) }}</div>
            </td>
            <td>
            ...
            <td>
              <span class="tag is-light">{{
                amountProductOrderBillsItemsTotal(order.bills)
              }}</span>
            </td>
            <td>
              <span
                ...
                :class="{
                  'is-danger is-light':
                    totalAmountBills(order.bills) == 0 &&
                    amountProductOrderBillsItemsTotal(order.bills) > 0,
                  'is-warning':
                    totalAmountBills(order.bills) > 0 &&
                    totalAmountBills(order.bills) <
                      amountProductOrderBillsItemsTotal(order.bills),
                  'is-warning is-light':
                    totalAmountBills(order.bills) == 0 &&
                    amountProductOrderBillsItemsTotal(order.bills) == 0,
                ...
                >{{ totalAmountBills(order.bills) }}</span
              >
            </td>
            <td>
