<template>
  <div class="container">
    <h1 class="is-size-3">Užsakymo redagavimas</h1>
<table class="table">
   <tr><td>
<div class="field is-grouped">
  ...
  <div class="field is-grouped">
     <div class="control">
          ...
          <input class="input" :class="errors.delivery_date && 'is-danger'" :disabled="status == 'done'" type="datetime-local" v-model="form.delivery_date">
          </div>
     <div class="control">
    ...
    <div class="control">
      ...
      <input class="input" :class="errors.note && 'is-danger'" style="width: 240px;" :disabled="status == 'done'" type="text" v-model="form.note">
    </div>
  </div>
  </td></tr>
  <tr>
  ...
  <div class="column">
    ...
    <div class="control">
      <input class="input" :disabled="status == 'done'" :class="errors.address && 'is-danger'" style="width: 180px;" type="text" v-model="form.address" :placeholder="client.address">
    </div>
  </div>
  <div class="column">
    <label class="label" for="city">Miestas:</label>
    <div class="control">
      <input class="input" :disabled="status == 'done'" :class="errors.city && 'is-danger'" style="width: 180px;" type="text" v-model="form.city" :placeholder="client.city">
    </div>
  </div>
  <div class="column">
    <label class="label" for="city">Pašto kodas:</label>
    <div class="control">
      <input class="input" :disabled="status == 'done'" :class="errors.postal_code && 'is-danger'" style="width: 180px;" type="text" v-model="form.postal_code" :placeholder="client.postal_code">
    </div>
  </div>
  <div class="column">
    <label class="label" for="country">Šalis:</label>
    <div class="control">
      <input class="input" :disabled="status == 'done'" :class="errors.country && 'is-danger'" style="width: 180px;" type="text" v-model="form.country" :placeholder="client.country">
    </div>
  </div>
  </div>
  </div>
...
<tr>
  ...
  <div class="column">
    ...
    <div class="column">
      <label class="label" for="address">Gatvė:</label>
    <div class="control">
      <input class="input" :disabled="status == 'done'" :class="errors.laddress && 'is-danger'" style="width: 180px;" type="text" v-model="form.laddress" placeholder="Žibilėlio g. 8">
    </div>
  </div>
  <div class="column">
    <label class="label" for="city">Miestas:</label>
    <div class="control">
      <input class="input" :disabled="status == 'done'" :class="errors.lcity && 'is-danger'" style="width: 180px;" type="text" v-model="form.lcity" placeholder="Užventė">
    </div>
  </div>
  <div class="column">
    <label class="label" for="city">Pašto kodas:</label>
    <div class="control">
      <input class="input" :disabled="status == 'done'" :class="errors.lpostal_code && 'is-danger'" style="width: 180px;" type="text" v-model="form.lpostal_code" placeholder="Mažeikių r.">
    </div>
  </div>
  <div class="column">
    <label class="label" for="country">Šalis:</label>
    <div class="control">
      <input class="input" :disabled="status == 'done'" :class="errors.lcountry && 'is-danger'" style="width: 180px;" type="text" v-model="form.lcountry" placeholder="Lietuva">
    </div>
  </div>
  </div>
  </div>
...
<table class="table is-fullwidth">
      ...
      <tbody>
        <tr v-for="(productRow, index) in fulfilledProductRows" v-bind:key="index">
          <td>
        ...
        <tr v-for="(productRow, index) in productRows" v-bind:key="index">
          ...
          <td>
            <div v-if="status != 'done'"><button @click="removeProductRow(index)" class="button is-danger">
              X
            </button></div>
          </td>
        </tr>
...
<script>
import { fetchPriceListsEnabled, fetchTransports, fetchOrder, fetchBlueprintsByClass, fetchProductClasses, fetchClients, fetchBlueprint, updateOrder, fetchClient } from '@/lib/api';
import { dateToLocaleString, sortBlueprintsByActualSizes } from '@/lib/ui';
...
export default {
  ...
  methods: {
    addProductRow(orderItem, fulfilled) {
      const productRow = {
        product_class_id: 0,
        blueprints: [],
        blueprint_id: 0,
      ...
      fetchBlueprintsByClass(productRow.product_class_id).then((res) => {
        ...
        if (fulfilled)
        this.fulfilledProductRows.push(productRow);
        if (this.productRows.length >= this.form.order_items.length && this.fulfilledProductRows.length >= this.form.order_item_fulfills.length)
        this.done = true;
        this.getTotalVolume();
        });
      });
    ...
    load() {
      ...
      fetchOrder(this.id).then((res) => {
        ...
        this.form.order_item_fulfills.forEach(orderItem => this.addProductRow(orderItem, true));
        if (this.form.order_items.length == 0 && this.form.order_item_fulfills.length == 0 && this.status == "done") this.done = true;
      });
    },
    ...
    getTotalVolume() {
      let volume = 0;
      for (let productRowIndex = 0; productRowIndex < this.fulfilledProductRows.length; productRowIndex++) {
        const productRow = this.fulfilledProductRows[productRowIndex];
   ...
   parseErrors(errors) {
    ...
    postForm(completeOrder) {
      if (completeOrder) this.form.completeOrder = true;
      this.form.productRows = this.productRows;
      this.form.transfers = undefined;
      this.form.order_item_fulfills = undefined;
      this.form.order_items = undefined;
      this.form.client = undefined;
      updateOrder(this.id, this.form)
        .then(res => {
          const content = res.data;
          this.parseErrors(content.errors);
          if (content.errors.length != 0) return;
          if (completeOrder) this.$router.go();
