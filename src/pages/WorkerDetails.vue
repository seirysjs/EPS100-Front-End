<template>
  <div class="container">
    <div class="block">
      <h1 class="is-size-3">
        Darbuotojas "{{ worker.name }}" atlikti ir priskirti darbai
      </h1>
      <div class="control">
        <router-link :to="{ name: 'workerList' }" class="button is-light"
          >Grįžti</router-link
        >
      </div>
    </div>
      ...
      >
          ...
          >
            <template #head>
              <tr>
                ...
                <th>
                  <label>Užsakymo&nbsp;Nr.,<br />Važtaraščio&nbsp;Nr.</label>
                </th>
              ...
              <tr v-for="transfer in data" v-bind:key="transfer.transfer_id">
                ...
                <td>
                  <router-link
                    :to="{
                      name: 'orderDetails',
                      params: { id: transfer.order.order_id },
                    }"
                ...
                <td>
                  ...
                  {{ transfer?.transport?.number }},<br />{{ worker.name }}
                </td>
                <td>
        ...
        <tab name="Blokų pjovimai" v-if="blockCuts.length != 0">
          ...
          <data-table
            :rowsPerPage="15"
            :list="blockCuts"
            @dataSlice="(data) => (this['data' + 'Cuts'] = data)"
          >
            <template #head>
              <tr>
                <th>Darbuotojas</th>
            ...
            <template #body>
              <tr
                v-for="warehouseItem in dataCuts"
              ...
              >
                ...
                <td>
                  <div class="tag">
                    {{
                      warehouseItem.block_id
                        ? warehouseItem.block_id
                        : "Grupė Nr. " + warehouseItem.block_multi_cut_id
                    }}
                  </div>
                </td>
                <td>
                ...
                <td>
                  <span class="tag is-light">x{{ warehouseItem.count }}</span>
                </td>
                <td>
                  <span class="tag is-light">{{
        ...
        <tab name="Blokai">
          ...
          <data-table
            :rowsPerPage="10"
            :list="blocks"
            @dataSlice="(data) => (this['data' + 'Blocks'] = data)"
          >
            <template #head>
              <tr>
                <th>Bloko Nr.</th>
            ...
            <template #body>
              <tr v-for="block in dataBlocks" v-bind:key="block.block_id">
                ...
                <td>
                  <div class="tag">{{ block.product_class.name }}</div>
                </td>
                <td>
                ...
                <td>
                  <span
                    class="tag is-light"
                    :class="{
                      'is-danger': block.status == 'void',
                      'is-warning':
                        block.status == 'drying' || block.status == 'wip',
                      'is-success': block.status == 'queue',
                    }"
                    >{{ block.status ? ltBlockStatus(block.status) : "" }}</span
                  >
                </td>
                <td>
                  <div class="field is-grouped">
...
import {
  fetchWorker,
  fetchTransfersByWorker,
  fetchBlockCutsByWorker,
} from "@/lib/api";
import {
