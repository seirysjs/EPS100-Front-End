  ...
  <div class="container">
    ...
    <div class="block">
      <data-table
        :rowsPerPage="10"
        :list="priceLists"
        @dataSlice="(data) => (this['data' + 'PriceLists'] = data)"
      >
        <template #head>
          <tr>
            <th>
              <label>Kainoraščio<br />Nr.</label>
            </th>
            <th>
              <label>Pasirinkimų<br />sąraše</label>
            </th>
            <th><label>Pavadinimas</label></th>
            <th><label>Papildomas aprašas</label></th>
            <th>
              <label>Kainoraščio<br />data</label>
            </th>
        ...
        <template #body>
          <tr
            v-for="priceList in dataPriceLists"
            v-bind:key="priceList.price_list_id"
          >
            <td>
              <router-link
                :to="{
                  name: 'priceListDetails',
                  params: { id: priceList.price_list_id },
                }"
            ...
            <td>
              <div class="tag">{{ priceList.orders.length }}</div>
            </td>
