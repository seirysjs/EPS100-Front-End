<template>
  <div class="container">
    <h1 class="is-size-3">
      Kainoraščio ({{ form.price_list_id }}) "{{ form.name }}" informacija
    </h1>

    <div class="block has-background-light p-5">
      <div class="field is-grouped is-grouped-left">
        <div class="control">
          <router-link :to="{ name: 'priceList' }" class="button is-light"
            >Grįžti</router-link
          >
        </div>
        <div class="control">
            ...
            <table class="table is-fullwidth">
              <thead>
                ...
                <tr>
                  ...
                  <th>
                    <label
                      >Bloko savikaina<br />(4.8m3)&nbsp;-&nbsp;(Eur)</label
                    >
                  </th>
                  <th>
                    <label
                      >Gaminio savikaina<br />(1m3)&nbsp;-&nbsp;(Eur)</label
                    >
                  </th>
              ...
              <tbody>
                <tr v-for="(price, index) in form.prices" v-bind:key="index">
                  ...
                  <td>
                    <div class="tag">
                      {{ roundNumberScaleTwo(price.amount * 4.8) }}
                    </div>
                  </td>
                </tr>
              </tbody>
                ...
                <tr v-for="order in data" v-bind:key="order.order_id">
                  ...
                  <td>
                    <span class="tag is-light">{{
                      roundNumber(
                        totalVolumeOrderItems(order.order_item_fulfills) +
                          totalVolumeOrderItems(order.order_items)
                      )
                    }}</span>
                  </td>
                  <td>
          ...
          <div class="block">
            ...
            >
              <template #head>
                <tr>
                  <th>
                    <label>Užsakymo&nbsp;Nr.,<br />Sąskaitos&nbsp;Nr.</label>
                  </th>
                  <th><label>Klientas</label></th>
                  <th><label>Dok. data</label></th>
                  <th><label>Produktai</label></th>
                  <th>
                    <label>Kiekis<br />(m3)</label>
                  </th>
                  <th>
                    <div style="text-align: right">
                      <label>Mokėjimai (Eur)</label>
                    </div>
                  </th>
              ...
              <template #body>
                <tr v-for="bill in dataBills" v-bind:key="bill.bill_id">
                  ...
                  <td>
                    <span class="tag is-light">{{
                      Math.round(
                        totalVolumeOrderItems(bill.bill_items) * 1000
                      ) / 1000
                    }}</span>
                  </td>
                  <td>
                    <div style="text-align: right">
                      <span
                        ...
                        :class="{
                          'is-danger':
                            totalAmountPayments(bill.bill_payments) == 0 &&
                            amountProductBillItemsTotal(bill) > 0,
                          'is-warning':
                            totalAmountPayments(bill.bill_payments) > 0 &&
...
import {
  fetchPriceList,
  fetchProductClasses,
  fetchOrdersByPriceList,
  fetchBillsByPriceList,
} from "@/lib/api";
import {
  dateToLocaleString,
