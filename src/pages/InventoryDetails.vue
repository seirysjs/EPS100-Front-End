<template>
  <div class="container">
    <h1 class="is-size-3">
      Produkto
      <span v-if="blueprint.product_class.name != ''"
        >({{ blueprint.product_class.name }} -
        {{ blueprint.product_size.x_mm }}x{{ blueprint.product_size.y_mm }}x{{
    ...
    <tabs
      v-if="orderItems.length != 0 || warehouseItems.length != 0"
    ...
    >
      <tab name="Užsakymai" v-if="orderItems.length != 0">
        ...
        <data-table
          :rowsPerPage="10"
          :list="orderItems"
          @dataSlice="(data) => (this['data' + 'Orders'] = data)"
        >
          <template #head>
            <tr>
              <th>Užsakymo Nr.</th>
          ...
          <template #body>
            <tr
              v-for="orderItem in dataOrders"
              v-bind:key="orderItem.order_item_id"
            >
              <td>
                <router-link
                  :to="{
                    name: 'orderDetails',
                    params: { id: orderItem.order.order_id },
                  }"
              ...
              <td>
                {{ orderItem.order?.transport?.name }}
                {{ orderItem.order?.transport?.number }}
              </td>
              <td>
                <span class="tag is-light">x{{ orderItem.count }}</span>
              </td>
              <td>
                <div class="tag">
                  {{ orderItem.blueprint.product_class.name }}
                </div>
                <br />
                <div class="field is-grouped">
                  <div class="control">
                    <div class="tag">
                      {{ orderItem.blueprint.product_size.x_mm }}
                    </div>
                    x
                    <div class="tag">
                      {{ orderItem.blueprint.product_size.y_mm }}
                    </div>
                    x
                    <div class="tag">
                      {{ orderItem.blueprint.product_size.z_mm }}
                    </div>
                  </div>
                </div>
              </td>
              ...
              <td>
                {{ dateToLocaleDateString(orderItem.order.delivery_date) }}
              </td>
              <td>
                <div class="field is-grouped">
                  <div class="control">
      ...
      <tab name="Inventorius" v-if="warehouseItems.length != 0">
        ...
        <data-table
          :rowsPerPage="10"
          :list="warehouseItems"
          @dataSlice="(data) => (this['data' + 'Inv'] = data)"
        >
          <template #head>
            <tr>
              <th>Darbuotojas</th>
          ...
          <template #body>
            <tr
              v-for="warehouseItem in dataInv"
