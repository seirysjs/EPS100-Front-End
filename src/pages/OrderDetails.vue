  ...
  <div class="container">
    ...
    <div class="block has-background-light p-5">
      <div class="field is-grouped is-grouped-left">
        <div class="control">
          <router-link :to="{ name: 'orderList' }" class="button is-light"
            >Grįžti</router-link
          >
        </div>
        <div class="control">
    ...
    <div class="block">
      <div class="field is-grouped">
        ...
        <div class="control">
          ...
          <input
            class="input"
            :class="errors.delivery_date && 'is-danger'"
            type="datetime-local"
    ...
    <tabs
      ...
      nav-class="field is-grouped is-grouped-left"
      nav-item-class="control"
      nav-item-link-class="button is-light"
    >
      <tab name="Važtaraščiai" v-if="transfers.length > 0">
        ...
        <div class="block">
          <data-table
            :rowsPerPage="5"
            :list="transfers"
            @dataSlice="(data) => (this['data' + 'Transfers'] = data)"
          >
            <template #head>
              <tr>
            ...
            <template #body>
              <tr
                v-for="transfer in dataTransfers"
                v-bind:key="transfer.transfer_id"
              >
                <td>
                  <div class="tag">{{ transfer.transfer_id }}</div>
                </td>
                <td>{{ transfer.vaz_number }}</td>
                <td>
                  {{
                    transfer.invoice_date
                      ? dateToLocaleDateString(transfer.invoice_date)
                      : dateToLocaleDateString(transfer.order.delivery_date)
                  }}
                </td>
                <td>
                  {{ transfer?.transport?.name }}
                  {{ transfer?.transport?.number }}
                </td>
                <td>{{ transfer?.worker?.name }}</td>
                <td>
                  <div class="tag">{{ transfer?.transfer_items?.length }}</div>
                </td>
                <td>
                  <span class="tag is-light">{{
                    Math.round(
                      totalVolumeOrderItems(transfer.transfer_items) * 1000
                    ) / 1000
                  }}</span>
                </td>
                <td>
                  <div class="field is-grouped">
                    ...
                    <div class="control">
                      <button
                        class="button is-light"
                        @click="invoice(transfer.transfer_id)"
                      >
                        Nuoroda
                      </button>
                    </div>
      ...
      <tab name="Produktai">
        <div v-if="productRows.length > 0">
          ...
          <div class="block">
            <data-table
              :rowsPerPage="5"
              :list="productRows"
              @dataSlice="(data) => (this['data' + 'Rows'] = data)"
            >
              <template #head>
                <tr>
              ...
              <template #body>
                <tr v-for="(productRow, index) in dataRows" v-bind:key="index">
                  <td>
                    <div class="tag">
                      {{ productRow.blueprint.product_class.name }}
                    </div>
                  </td>
                  <td>
                    <div class="tag">
                      {{ productRow.blueprint.product_size.x_mm }}
                    </div>
                    x
                    <div class="tag">
                    ...
                    x
                    <div class="tag">
                      {{ productRow.blueprint.product_size.z_mm }}
                    </div>
                  </td>
                  <td>
                    x
                    <div class="tag">{{ productRow.count }}</div>
                  </td>
                  <td>
                    <span class="tag is-light">{{
                      Math.round(
                    ...
                    <div class="field">
                      <div class="control">
                        <h3>
                          <b>Iš viso: </b
                          ><span class="tag is-light">{{
                            totalVolumeOrderItems(form.order_items)
                          }}</span
                          >&nbsp;<b>(m3)</b>
                        </h3>
                      </div>
        ...
        <div v-if="orderItemFulfills.length > 0">
          ...
          <div class="block">
            <data-table
              :rowsPerPage="5"
              :list="orderItemFulfills"
              @dataSlice="(data) => (this['data' + 'Fulfilled'] = data)"
            >
              <template #head>
                <tr>
                  <th><label>Klasė</label></th>
              ...
              <template #body>
                <tr
                  v-for="(productRow, index) in dataFulfilled"
                  v-bind:key="index"
                >
                  <td>
                    <div class="tag">
                  ...
                  <td>
                    {{ dateToLocaleDateString(productRow.fulfilled_date) }}
                  </td>
                  <td>
                    x
                    <div class="tag">{{ productRow.count }}</div>
                    ...
                    <div class="field">
                      <div class="control">
                        <h3>
                          <b>Iš viso: </b
                          ><span class="tag is-light">{{
                            totalVolumeOrderItems(form.order_item_fulfills)
                          }}</span
                          >&nbsp;<b>(m3)</b>
                        </h3>
                      </div>
        ...
        <div v-if="orderItemFulfills.length == 0 && productRows.length == 0">
          <h3 class="is-size-4">Produktų sąrašas tuščias.</h3>
        </div>
      </tab>
      ...
      <tab name="Sąskaitos">
          ...
          <div class="control">
            <h3>
              ...
              ><span class="tag is-success is-light">
                {{ amountProductOrderBillsItemsTotal(bills) }} </span
              ><b>&nbsp;(Eur)<br /></b>
            </h3>
          </div>
          <div class="control">
            <h3>
              ...
              ><span
                ...
                :class="{
                  'is-danger':
                    totalAmountBills(bills) == 0 &&
                    amountProductOrderBillsItemsTotal(bills) > 0,
                  'is-warning':
                    totalAmountBills(bills) > 0 &&
              ...
              >
                {{ totalAmountBills(bills) }} </span
              ><b>&nbsp;(Eur)<br /></b>
            </h3>
          </div>
          <div class="control">
            <h3>
              <b>Mokėjimų likutis:&nbsp;</b
              ><span
                class="tag"
                :class="{
                  'is-danger':
        ...
        <div class="block">
          <data-table
            :rowsPerPage="5"
            :list="bills"
            @dataSlice="(data) => (this['data' + 'Bills'] = data)"
          >
            <template #head>
              <tr>
            ...
            <template #body>
              <tr v-for="bill in dataBills" v-bind:key="bill.bill_id">
                ...
                <td>
                  {{
                    bill.bill_date
                      ? dateToLocaleDateString(bill.bill_date)
                      : dateToLocaleDateString(bill.order.delivery_date)
                  }}
                </td>
                <td>
                  <div class="tag">{{ bill.bill_items?.length }}</div>
                </td>
                <td>
                  <span class="tag is-light">{{
                    Math.round(totalVolumeOrderItems(bill.bill_items) * 1000) /
                    1000
                  }}</span>
                </td>
                <td>
...
import {
  ...
  amountProductOrderBillsItemsTotal,
  totalAmountBills,
  totalAmountPayments,
  countVolumeBlueprint,
  dateToLocaleString,
  totalVolumeOrderItems,
  totalVolumeTransfers,
  totalVolumeBills,
