<template>
  <div class="container">
    <h1 class="is-size-3">Kainoraščio ({{ form.price_list_id }}) "{{ form.name }}" forma</h1>
...
<script>
import { fetchPriceList, updatePriceList, fetchProductClasses } from '@/lib/api';
  ...
  methods: {
    load() {
      ...
      fetchPriceList(this.id).then(res => {
        this.form = res.data;
        this.form.price_list_date = dateToLocaleString(res.data.price_list_date);
        
        for (let indexProductClass = 0; indexProductClass < this.form.prices.length; indexProductClass++) {
    ...
    addProductClassPriceRow(productClassPrice) {
      ...
      productClassRow.markup = productClassPrice.markup ? parseFloat(productClassPrice.markup) : 20;
      productClassRow.amountProduct = productClassPrice.amount ? parseFloat(productClassPrice.amount) : 0;

      productClassRow.amountProductBlock = roundNumberScaleTwo(productClassRow.amountProduct * 4.8);

      productClassRow.amountCost = roundNumberScaleTwo((productClassRow.amountProduct * 100) / (100 + productClassRow.markup));

      productClassRow.amountCostBlock = roundNumberScaleTwo(productClassRow.amountCost * 4.8);
    ...
    postForm() {
      ...
      updatePriceList(this.id, this.form)
        .then(res => {
          ...
          this.$router.push({ name: 'priceListDetails', params: { id: this.id } });
        })
