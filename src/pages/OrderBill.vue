  ...
  methods: {
    ...
    load() {
      ...
      fetchOrder(this.id).then((res) => {
        ...
        fetchBillNewId().then((res) => {
          this.bill_id = res.data;
          this.form.bill_id = this.bill_id;
        });

        this.form.bill_date = dateToLocaleString(this.form.delivery_date);
        this.form.days_postponed = 7;

        this.onChangeClient();

        fetchOrderItemsForBillByOrder(this.id).then((res) => {
