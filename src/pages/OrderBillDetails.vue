  ...
  <div class="container">
    ...
    <div class="block has-background-light p-5">
      <div class="field is-grouped is-grouped-left">
        <div class="control">
          <router-link :to="{ name: 'orderBillList' }" class="button is-light"
            >Grįžti</router-link
          >
        </div>
        <div class="control">
      ...
      <div class="field is-grouped">
        ...
        <div class="control">
          <label class="label" for="drying_started_at"
            >Mokėjimo terminas:</label
          >
          <input
            class="input"
            :class="errors.payment_due && 'is-danger'"
            type="text"
            v-model="payment_due"
            disabled
          />
                  ...
                  <div class="field">
                    <div class="control">
                      <h3>
                        <b>Iš viso </b
                        ><span class="tag is-light">{{
                          totalVolumeOrderItems(form.bill_items)
                        }}</span
                        >&nbsp;<b>(m3)</b>
                      </h3>
                    </div>
      ...
      <tab name="Mokėjimai">
          ...
          <div class="control">
            <h3>
              ...
              ><span class="tag is-success is-light">
                {{ amountProductBillItemsTotal(form) }} </span
              ><b>&nbsp;(Eur)<br /></b>
            </h3>
          </div>
          <div class="control">
            <h3>
              ...
              ><span
                ...
                :class="{
                  'is-danger':
                    totalAmountPayments(billPayments) == 0 &&
                    amountProductBillItemsTotal(form) > 0,
                  'is-warning':
                    totalAmountPayments(billPayments) > 0 &&
              ...
              >
                {{ totalAmountPayments(billPayments) }} </span
              ><b>&nbsp;(Eur)<br /></b>
            </h3>
          </div>
          <div class="control">
        ...
        <div class="block">
          <data-table
            :rowsPerPage="5"
            :list="billPayments"
            @dataSlice="(data) => (this['data' + 'BillPayments'] = data)"
          >
            <template #head>
              <tr>
            ...
            <template #body>
              <tr
                v-for="billPayment in dataBillPayments"
                v-bind:key="billPayment.bill_payment_id"
              >
                <td>
                  <router-link
                    :to="{
                      name: 'billPaymentEdit',
                      params: { id: billPayment.bill_payment_id },
                    }"
                ...
                <td>
                  <span class="tag is-light">{{ billPayment.bill_id }}</span>
                </td>
                <td>{{ billPayment.note }}</td>
                <td>{{ dateToLocaleDateString(billPayment.payment_date) }}</td>
                <td>
                  <span class="tag is-light">{{ billPayment.amount }}</span>
                </td>
                <td>
                  <div class="field is-grouped">
                    <div class="control">
...
<script>
import { fetchBill, fetchClients, fetchBillPaymentsByBill } from "@/lib/api";
import {
  roundNumberScaleTwo,
  ...
  methods: {
    load() {
      ...
      fetchBill(this.id).then((res) => {
        this.form = res.data;
        this.form.bill_date = dateToLocaleString(
          res.data.bill_date ? res.data.bill_date : res.data.order.delivery_date
        );
        this.productRows = this.form.bill_items;
        this.payment_due = `${billPaymentsDue(res.data)} (${
          this.form.days_postponed
        }D)`;
