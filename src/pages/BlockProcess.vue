<template>
  <div class="container">
          ...
          <td colspan="4">
            <div class="field is-grouped is-grouped-right">
              <div class="control" style="margin-top: 10px">
                <b>Iš viso: </b><span class="tag is-light">{{ volume }}</span
                >&nbsp;<b>(m3)</b>
              </div>
              <div class="control">
    ...
    <div class="field is-grouped is-grouped-left">
      ...
      <div class="control">
        <button class="button is-link" @click="postForm()">
          Registruoti pjovimą
        </button>
      </div>
    </div>
  ...
  methods: {
    ...
    getTotalVolume() {
      ...
      for (
        ...
        productRowIndex < this.cuts.length;
        productRowIndex++
      ) {
    ...
    onChangeBlueprint(index) {
      ...
      if (this.cuts[index].blueprint_id != 0)
        fetchBlueprint(cut.blueprint_id).then((res) => {
          ...
          this.cuts[index].quantityM3 =
            Math.round(
              this.cuts[index].quantity * this.cuts[index].step * 1000
            ) / 1000;
          this.getTotalVolume();
        });
      ...
      if (this.cuts[index].blueprint_id == 0) {
        ...
        this.cuts[index].quantityM3 =
          Math.round(this.cuts[index].quantity * this.cuts[index].step * 1000) /
          1000;
        this.getTotalVolume();
      }
    },
    ...
    onChangeQuantity(index) {
      ...
      if (this.cuts[index].blueprint_id == 0)
        this.cuts[index].quantityM3 =
          Math.round(this.cuts[index].quantity * 0.001 * 1000) / 1000;
    ...
    onChangeQuantityM3(index) {
      ...
      if (this.cuts[index].blueprint_id == 0)
        this.cuts[index].quantity =
          Math.round((this.cuts[index].quantityM3 / 0.001) * 1000) / 1000;
    ...
    postForm() {
      processBlock(this.id, this.cuts)
        .then((res) => {
          const content = res;
          if (content.data.block) this.$router.push({ name: "blockList" });
        })
