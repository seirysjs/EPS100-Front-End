...
export const dateToLocaleDateString = (datetime) => {
  if (datetime) return new Date(datetime).toLocaleString("lt-LT").split(" ")[0];
};
...
export const sortInvBlueprintsByActualSizes = (warehouseItems) => {
  warehouseItems.sort(
    (a, b) => a.blueprint.product_size.z_mm - b.blueprint.product_size.z_mm
  );
  warehouseItems.sort(
    (a, b) => a.blueprint.product_size.y_mm - b.blueprint.product_size.y_mm
  );
